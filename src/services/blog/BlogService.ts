...
import { mapWordpressPost } from "./ProjectService";


const API_URL = import.meta.env.PUBLIC_WORDPRESS_API_URL;

let cachedCategories: Promise<Category[]> | null = null;
const cachedLimitedPosts: Record<number, Promise<{ posts: BlogPost[]; totalItem: number; totalPages: number }>> = {};
...
export async function getPostPaths() {
    const url = API_URL.replace("?rest_route=/wp/v2/posts", "/?rest_route=/wp/v2/posts") + "&_embed&per_page=100";
    const posts = await get<any[]>(url);

    // 🔹 Filtrar posts que pertenecen a proyectos u otras categorías que no son del blog
    const filtered = posts.filter((p) => {
        const termsGroups = p._embedded?.["wp:term"];
        if (!Array.isArray(termsGroups)) return true;
        const flatTerms = termsGroups.flat();
        return !flatTerms.some((t: any) =>
            t?.slug === "projects" ||
            t?.slug === "_servicesonly" ||
            t?.slug === "_highlights" ||
            t?.slug === "_skillsonly"
        );
    });
...
export async function fetchLimitedPosts(
    ...
    if (cachedLimitedPosts[limit]) return cachedLimitedPosts[limit];

    cachedLimitedPosts[limit] = (async () => {
        try {
    ...
    return cachedLimitedPosts[limit];
}
